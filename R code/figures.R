fig1_man <- function(){
  ...
  zs_kal_df <- ggplot2::fortify(zs, region="ZS") %>%
            dplyr::filter(id %in% c("KALEMIE", "NYEMBA"))

  zs_df <- ggplot2::fortify(zs, region="ZS")
  ...
  lakes <- lakes[lakes$NAME_OF_WA %in% c("Tanganyika", "Victoria", "Albert", "Edward", "Kivu", "Mweru", "Malawi", "Turkana", "Rukwa (north)", "Rukwa (south)", "Kyoga"),] %>%
              ggplot2::fortify(., region="NAME_OF_WA")

  box_zs <- sp::bbox(zs[zs$ZS %in% c("KALEMIE", "NYEMBA"),])
  ...
  drc_map_alt <- ggplot2::ggplot() +
              ...
              ggplot2::geom_path(
                ...
                size=0.5) +
              ggplot2::geom_rect(
                ggplot2::aes(xmin=box_zs[1,1], xmax=box_zs[1,2], ymin=box_zs[2,1], ymax=box_zs[2,2]),
                color="red",
                fill=NA,
                size=1) +
              ggplot2::geom_point(
...
fig2 <- function(){
  ...
  # load("PATH TO THE FOLDER WHERE YOU WANT TO SAVE THE ESTIMATES/estimate.rda")
...
fig3 <- function(){
  ...
  load("PATH ON YOUR MACHINE/data/vacci_res.rda")
  vacci_opt <- vacci_res %>%
                dplyr::left_join(
                  .,
                  date_iso_l %>%
...
fig4 <- function(){
  ...
  # load("PATH TO THE FOLDER WHERE YOU WANT TO SAVE THE ESTIMATES/contrib.rda")

  # No Interhuman and no env
  ...
  # env_sample <- build_sample2(name=paste0(c("theta1_", "theta2_", "theta3_"), "0_7_s1_40_migr"))
  ...
  # beta_h <- mean(exp(theta1_sample$alpha0))
