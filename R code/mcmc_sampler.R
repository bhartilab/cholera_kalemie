...
# replace the path toward the script "other_functions.R" on your machine
source("XXXX/R code/other_functions.R")
...
# 100000 burnin corresponds to our original 40000 + the extra trimming of 60000
burn.in=100000
...
theta.1.curr=c(-0.3430312, 0.6721811, 0.3318278, 0.008952414, 0.270325, 0.3623217, 0.05620103, 0.003411574, 1.495769, 1.468861, 0.5324734, 0.4909023)
theta.2.curr=c(4285.66867,75.35729,1.00540)
theta.3.curr=c(0.01736722, -2.466238)

# Adjust based on the assumption on the ratio between susceptible and infected among mobile people
ratio_S_I = 40
...
theta.1.prior.mean = c(0,0,0,0,0,0,0,1/(5*52),0,1.4,0,0)

theta.1.lower = c( -Inf,0,-Inf,0,0,0,0,1/(8*52),-Inf,1.15,0,0)
theta.1.upper = c( Inf,Inf,Inf,Inf,Inf,Inf,2,1/52,Inf,1.65,Inf,Inf)
...
# the function dXdt is defined in "other_functions.R"
